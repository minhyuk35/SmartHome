...
이 노트북들은 POP 하드웨어가 있는 라즈베리 파이(또는 호환 보드)에서 실행을 전제로 함. PC(Java/Python)와 연결할 때 필요한 설정만 정리.

## 네트워크/포트
- PC IP: `127.0.0.1` 를 기본으로 사용. POP가 다른 네트워크라면 노트북의 `JAVA_IP` 값을 PC 주소로 변경.
- 포트(Java GUI 기준):  
  - 명령 39186, 센서 39187, 도어 이벤트 39189
...
- `Door.ipynb`: 도어락/키패드/PIR/부저/서보 제어.  
  ...
  - 도어 이벤트 송신: 39189(TCP)로 `LOCKED`/`UNLOCKED`/`ALERT_FAIL_3`.  
...
## 동작 순서(라즈베리 파이 쪽)
1. PC에서 Python `main.py`, Java GUI(`java Main`) 실행.  
2. 라즈베리 파이에서 `Data_TCP.ipynb` 실행 → 센서 데이터가 Java로 전송(39187).  
3. 도어락/키패드까지 쓸 경우 `Door.ipynb` 실행 → 명령 수신(39186)/도어 이벤트 송신(39189)/PIR 포함.  
4. (선택) Spring 대시보드를 사용할 경우에는 별도로 `spring-app`을 실행해 WebSocket으로 중계한다.
